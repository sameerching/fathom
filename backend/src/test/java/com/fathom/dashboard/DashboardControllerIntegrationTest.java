...
class DashboardControllerIntegrationTest {
    ...
    void categoryBreakdownCalculation() throws Exception {
        ...
        String systemCategories = mockMvc.perform(get("/api/categories/system"))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        JsonNode categories = objectMapper.readTree(systemCategories);
        JsonNode foodCategory = null;
        for (JsonNode category : categories) {
            if ("Food".equalsIgnoreCase(category.get("name").asText())) {
                foodCategory = category;
                break;
            }
        }
        assertNotNull(foodCategory, "Expected system category named Food to exist");
        String categoryId = foodCategory.get("id").asText();
    ...
    void netWorthSummaryCalculation() throws Exception {
        ...
        mockMvc.perform(post("/api/users/{u}/investment-holdings", userId).contentType(MediaType.APPLICATION_JSON).content("{\"assetType\":\"MUTUAL_FUND\",\"name\":\"MF1\",\"investedAmount\":1000,\"currentValue\":1200,\"active\":true}"));
